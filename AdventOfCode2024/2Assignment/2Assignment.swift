...
final class Assignment2 {
    ...
    func calculateCollumnSum(_ matrix: [[Int]]) -> Int {
        var reports = 0
        for row in matrix {
            if isRowSafe(row) {
                reports += 1
    ...
    private func isRowSafe(_ row: [Int]) -> Bool {
        let increasing = zip(row, row.dropFirst()).allSatisfy { $1 > $0 && ($1 - $0) >= 1 && ($1 - $0) <= 3 }
        let decreasing = zip(row, row.dropFirst()).allSatisfy { $1 < $0 && ($0 - $1) >= 1 && ($0 - $1) <= 3 }
